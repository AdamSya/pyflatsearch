...
def user_input():
    ...
    # inputdict['furnishTypes'] = 'furnished'
    # inputdict['letType'] = 'longTerm'
    # inputdict['includeLetAgreed'] = 'false'
...
def rightmove_scraper():
    ...
    #   "find.html?locationIdentifier=REGION%5E87490&" +"maxPrice=%d&" % inputdict['maxprice'] + \
    #       "radius=%.1f&" % radius + \
...
def map_plotter():
    ...
    # plot_markersize = [15+(100/(1+np.e**(6*a-2))) for a in selected_properties['distance']]
    ...
    # plotdirs = {'png':'output/' + str(datetime.now())[:10] + '_' + str(inputdict['maxprice']) + '_' + str(proptype) + '.png',
    #             'pdf':'output/' + str(datetime.now())[:10] + '_' + str(inputdict['maxprice']) + '_' + str(proptype) + '.pdf',
    #             'svg': 'output/' + str(datetime.now())[:10] + '_' + str(inputdict['maxprice']) + '_' + str(proptype) + '.svg',
    #             'jpeg': 'output/' + str(datetime.now())[:10] + '_' + str(inputdict['maxprice']) + '_' + str(proptype) + '.jpeg'}
...
# min_price, inputdict['maxprice, radius, proptype, min_number_bedrooms, max_number_bedrooms = user_input()
...
# y = [1,5,10,32,13,40,83,166,299,299,1049,730]
# x = [9.43,7.99,12.16,22.04,13.69,27.64,47.04,81.96,144.19,169.57,428.29,347.92]
#
# p = np.polyfit(x,y,1)
# print(p)
# plt.close()
